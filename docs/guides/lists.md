# Lists

## Related Actions

[Activate GameObjects In List](../reference.md#activate-gameobjects-in-list)  
[Add Hierarchy To List](../reference.md#add-hierarchy-to-list)  
[Add Item To List](../reference.md#add-item-to-list)  
[Add Selection To List](../reference.md#add-selection-to-list)  
[Clear List](../reference.md#clear-list)  
[Deactivate GameObjects In List](../reference.md#deactivate-gameobjects-in-list)  
[Destroy Game Objects In List](../reference.md#destroy-gameobjects-in-list)  
[Disable Behaviours In List](../reference.md#disable-behaviours-in-list)  
[Disable Components In List](../reference.md#disable-components-in-list)  
[Enable Behaviours In List](../reference.md#enable-behaviours-in-list)  
[Enable Components In List](../reference.md#enable-components-in-list)  
[Get Farthest](../reference.md#get-farthest)  
[Get First Item From List](../reference.md#get-first-item-from-list)  
[Get Index Of Item In List](../reference.md#get-index-of-item-in-list)  
[Get Item From List](../reference.md#get-item-from-list)  
[Get JSON From List](../reference.md#get-json-from-list)  
[Get List Count](../reference.md#get-list-count)  
[Get Nearest](../reference.md#get-nearest)  
[Get Random Item From List](../reference.md#get-random-item-from-list)  
[Get Random Point In Collider List](../reference.md#get-random-point-in-collider-list) [Insert Item In List](../reference.md#insert-item-in-list)  
[Remove Item From List](../reference.md#remove-item-from-list)  
[Resize List](../reference.md#resize-list)  
[Send Command To List](../reference.md#send-command-to-list)  
[Set Item In List](../reference.md#set-item-in-list)  
[Shuffle List](../reference.md#shuffle-list)  
[Sleep List](../reference.md#sleep-list)  
[Sort List](../reference.md#sort-list)  
[Wake Up List](../reference.md#wake-up-list)

## Related Conditions

[Item Count Condition](../reference.md#item-count-condition)  
[List Condition](../reference.md#list-condition)

